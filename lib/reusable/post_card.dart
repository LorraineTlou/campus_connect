          ...
          children: [
            ...
            Text(
              post.commentList.isEmpty
                  ? 'Comment'
                  : '${post.commentList.length} Comment${post.commentList.length == 1 ? '' : 's'}',
              style: const TextStyle(
                fontSize: 13,
                fontWeight: FontWeight.w600,
                color: Color(0xFF90A4AE),
...
class _CommentSheetState extends State<_CommentSheet> {
  ...
  void _send() {
    if (!_canSend) return;
    if (_replyingTo != null) {
      context.read<PostProvider>().addReply(
        widget.post.id,
        _replyingTo!.id,
        _ctrl.text,
      );
  ...
  Widget build(BuildContext context) {
    return Consumer<PostProvider>(
      builder: (ctx, provider, _) {
        // Get the live post from provider
                  ...
                  child: livePost.commentList.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (_, i) => _CommentTile(
                            comment: livePost.commentList[i],
                            onReply: (comment) =>
...
class _CommentTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        _Avatar(
          initials: comment.author.substring(0, 1).toUpperCase(),
          size: 32,
        ),
        const SizedBox(width: 10),
        Expanded(
